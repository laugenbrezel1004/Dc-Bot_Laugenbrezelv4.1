...
import de.laurenzschmidt.json.WeatherData;
...
public class GetWeatherCommand extends Command {
    ...
    void execute(SlashCommandInteractionEvent event) {
        ...
        String apiUrl = "https://api.weatherbit.io/v2.0/current?city=" + city + "&key=" + this.apiKey;
        try {
...
//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! currentDate hinter : -> h in uhrzeit  2024-01-17:12

          //  currentDate = currentDate.replace(":", " um ");

            String user = String.valueOf(event.getMember().getUser().getName());
            botschaft.append("Hallo " + user).append(",\nin der Stadt ").append(cityName).append(" beginnt ein neuer Tag mit einem atemberaubenden Sonnenaufgang um ").append(sunriseTime2)
                    ...
                    .append("% Wolken, was auf eine ").append(description).append(" Szenerie hindeutet.\nDie aktuelle Temperatur beträgt ").append(temperature)
                    .append(" °C. " + temperatureOut).append(" Der Wind weht mit einer Geschwindigkeit von ")
                    .append(windSpeed).append(" km/h, wodurch " + windSpeedOut + " entsteht. Trotz einiger Wolken ist die UV-Strahlung mit einem Wert von ")
                    .append(uv).append(" auf einem moderaten Niveau, aber es wird empfohlen, angemessene Sonnenschutzmaßnahmen zu ergreifen.\n")
                    .append("\nDer Zeitpunkt dieser Messwerte ist am ").append(currentDate + " Uhr").append(" gewesen, und es scheint, als wäre es ein perfekter Tag, um die Schönheit der Natur zu genießen." );
